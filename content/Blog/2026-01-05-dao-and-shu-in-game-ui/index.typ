...
- 符号的二义性。左侧的导航栏中的图标与地图中的图标符号发生了复用。这会让玩家误以为右侧导航栏中的按钮是图例。如果它们真的是图例，那么左下角对于地点的颜色标识也属于图例，应当合并到一起。否则，它们应当使用不同的符号，并且把图例统一放置在地图附近的区域，当鼠标在图例上悬停时，地图上相应的元素需要高亮。
